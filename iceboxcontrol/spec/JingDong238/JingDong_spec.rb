...
APP_PATH = '../../IceBoxControl_JingDong_20150806_debug.apk'
	...
	context 'after user login as STA mode' do
		...
		context 'setting_page' do
			before :all do
				...
				@jd_dev_connect = id('com.iceboxcontrol:id/jd_dev_connect')
				@jd_dev_connect_textview_text = id('com.iceboxcontrol:id/jdDevices')
			...
			it 'check the setting page elements' do
				...
				expect((@jd_dev_connect_textview_text.text) == '京东设备连接').to be true
			...
			context 'userinfo page' do
				...
				end #change userinfo_password page
			end #userinfo page
			...
			end #network_setting page

			context 'delete all devices before add' do
...
#=begin
			context 'jd_dev_connect page' do
				it 'cancel to enter jd_dev_connect page' do
					@jd_dev_connect.click
					button('取消').click
					expect(@jd_dev_connect_textview_text.displayed?).to be true
				end
				context 'net_setting page' do
					before :each do
						...
						@wifi_pwd_input= id('com.iceboxcontrol:id/network_setting_vericode_remote_pwd_input')
						@network_setting_confirm = button('确定')
					end
					after :each do
					...
					it 'check the net_setting page elements' do
						...
						expect(@wifi_pwd_input.displayed?).to be true
						expect(@network_setting_confirm.displayed?).to be true
					end
					it 'set wifi connect without wifi name & password' do
						@network_setting_confirm.click
						expect(button('确定').displayed?).to be true
					end
					it 'set wifi connect without wifi name' do
						@wifi_pwd_input.type @wifi_right_pwd
						@network_setting_confirm.click
						expect(button('确定').displayed?).to be true
					end
					it 'set wifi connect without wifi password' do
					...
					#下面一个用例存在设计缺陷，即设置路由SSID和密码后，不会先验证密码是否正确。
					# it 'set wifi connect with wrong wifi password' do
					#    @wifi_setting_button.click
					#    text(@wifi_name).click
					#    @wifi_pwd_input.clear
					#    @wifi_pwd_input.type @wifi_wrong_pwd
					#    @network_setting_confirm.click
					#    expect(button('确定').displayed?).to be true
					# end
					it 'set wifi connect with rigth wifi name & password' do
						@wifi_setting_button.click
						text(@wifi_name).click
						sleep 3
						@wifi_pwd_input.clear
						...
						@network_setting_confirm.click
						set_wait(120)
						expect(button('设备联网成功，请点击进行设备绑定').displayed?).to be true
					end
				end #net_setting page
			end #jd_dev_connect page
#=end
		...
		context 'foodmanage_page' do
			...
			end #foodadd page->voiceInput page
