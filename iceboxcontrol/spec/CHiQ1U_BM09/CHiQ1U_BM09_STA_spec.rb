...
describe 'iceboxcontrol_CHiQ1U_BM09_STA_mode' do
	before(:all) do
		...
		context 'foodmanage_page' do
			...
			context 'foodadd detail_page' do
				...
				it 'foodadd_detail mode have two values for #560 series'do
					1.times {@foodadd_detail_spinner[1].click} #app的bug，前一次没有修改，则需要点击2次
			...
			context 'food_edit page' do
				...
				it 'changes take no effect if canceled' do
					...
					button('完成').click
					button('取消').click
					expect(ids('com.iceboxcontrol:id/foodmanage_item_Unit')[5].text).to eq('个')
				end
				it 'food unit can be edit' do
					...
					button('完成').click
					button('确定').click
					expect(ids('com.iceboxcontrol:id/foodmanage_item_Unit')[5].text).to eq('千克')
				end
